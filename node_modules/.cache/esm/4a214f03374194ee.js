let fs;_374‍.w("memfs",[["default",["fs"],function(v){fs=v}]]);require = require("esm")(module /*, options*/);
...
app.listen(port, () => _374‍.g.console.log(path.join(__dirname, 'static'))) //prints path to console

//API End Point
var busboy = require('connect-busboy'); //middleware for form/file upload
//var fs = require('memfs');       //File System - for file manipulation
app.use(busboy());

async function createMap(fileContents) {
    ...
    try {
        ...
        _374‍.g.console.log(err)
    }

    vcf.read("/userVCF.vcf");
    var vcfMap = new Map();
    //var vcfArray = new Array();
    var allFeatures = [];
    //setTimeout(function () {
    vcf.on('data', (f) => allFeatures.push(f));
    vcf.on('end', () => _374‍.g.console.log("all features: " + allFeatures));
    //}, 0)
    /*
    ...
    */
    vcf.on('error', function (err) {
        _374‍.g.console.error('it\'s not a vcf', err)
    })
...
app.get('/calculate_score/', async function (req, res) {
    ...
    var snpMapList = await createMap();
        ...
        sequelize
            ...
            .then(() => {
                    ...
                    personResultJsons.push(Promise.all(resultsArray).then(resultsArray => {
                        ...
                        resultsArray.forEach(function (response) {
                            ...
                            if (response.length > 1) {
                                ...
                                _374‍.g.console.log(response);
                            }
                            if (response.length > 0) {
                                ORs.push(response[0].oddsRatio);
                                _374‍.g.console.log(response[0].oddsRatio);
                            }
                        });
                ...
                Promise.all(personResultJsons).then(jsons => {
                    _374‍.g.console.log(jsons);
                    res.send(jsons);
                }).catch(err => {
                    res.status(500).send(err);
                    _374‍.g.console.error('Something went wrong. Error sent to client too.', err);
                });
            })
            .catch(err => {
                res.status(500).send(err);
