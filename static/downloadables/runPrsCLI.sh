    ...
    do 
        ...
        case $option in 
            ...
            10 ) echo -e "${MYSTERYCOLOR} -v: ${NC}"
                ...
                echo "If this parameter is not included, the default CSV result file will include the study ID"
		...
		echo "If the output file is in JSON format, the results will, by default, be in verbose format."
	        echo "Condensed JSON result files are not available"	
                echo "" ;;
        ...
        case $c in 
            ...
            o)  if ! [ -z "$output" ]; then
                ...
                if ! [[ "$output" =~ .csv$|.json$ ]]; then
                    ...
                    echo -e "Valid formats are ${GREEN}csv${NC} or ${GREEN}json${NC}"
                    exit 1
                fi;;
            c)  if ! [ -z "$cutoff" ]; then
