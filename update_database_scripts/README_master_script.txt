...
normalizeChars.py- a Python script that normalizes the characters in the associations table in place using unicodedata.normalize()

filterBetaUnits.py- a Python script that finds trait/pValAnno/betaAnno/valType/studyID combination groups with more than one beta unit associated with
    them and removes all associations in that group, printing the number of associations removed and the study IDs affected.
...
createServerAssociClumpsAndTraitStudyToSnpFiles.py- A Python script that generates association and clump files that can be downloaded using the CLI. The generated files
