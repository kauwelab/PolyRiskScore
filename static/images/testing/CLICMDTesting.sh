...
for i in "sample.vcf" "sample.txt"; do
    ...
    # test 24 (-t x2, -k x2)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test24.csv" -c $pvalue -r $refGen -p $pop -t "Insomnia" - t "acne" -k "HI" -k "O"

    # test 25 (-t, -k, -s) #TODO Maddy, did you use "-s 1" or "-s 2"
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test25.csv" -c $pvalue -r $refGen -p $pop -t "Acne" -k "HI" -s "1"

    # TODO test 26 (test 23 without internet)

    # test 27 (-t, -i)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test27.csv" -c $pvalue -r $refGen -p $pop -t "Alzheimer's disease" -i "GCST000010"

    # test 28 (-t x2, -i x2)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test28.csv" -c $pvalue -r $refGen -p $pop -t "alzheimer's disease" -t "acne" -i "GCST000010" -i "GCST000001"

    # test 29 (-t, -i, -s) #TODO Maddy, did you use "-s 1" or "-s 2"
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test29.csv" -c $pvalue -r $refGen -p $pop -t "Asthma" -i "GCST000001" -s "1"

    # TODO test 30 (test 27 without internet)

    # test 31 (-k, -e)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test31.csv" -c $pvalue -r $refGen -p $pop -t "Asthma" -e "East Asian"

    # test 32 (-k x2, -e x2)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test32.csv" -c $pvalue -r $refGen -p $pop -t "Asthma" -t "Alzheimer's disease" -i "East asian" -i "european"

    # test 33 (-k, -e, -s) #TODO Maddy, did you use "-s 1" or "-s 2"
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test33.csv" -c $pvalue -r $refGen -p $pop -t "Asthma" -e "East Asian" -s "1"

    # TODO test 34 (test 31 without internet)

    # test 35 (-k, -i)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test35.csv" -c $pvalue -r $refGen -p $pop -k "HI" -i "GCST000001"

    # test 36 (-k x2, -i x2)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test36.csv" -c $pvalue -r $refGen -p $pop -k "HI" -k "LC" -i "GCST000001" -i "GCST000010"

    # test 37 (-k, -i, -s) #TODO Maddy, did you use "-s 1" or "-s 2"
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test37.csv" -c $pvalue -r $refGen -p $pop -k "HI" -i "GCST000001" -s "1"

    # TODO test 38 (test 35 without internet)

    # test 39 (-k, -e)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test39.csv" -c $pvalue -r $refGen -p $pop -k "HI" -e "African"

    # test 40 (-k x2, -e x2)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test40.csv" -c $pvalue -r $refGen -p $pop -k "HI" -k "LC" -e "African" -e "East Asian"

    # test 41 (-k, -e, -s) #TODO Maddy, did you use "-s 1" or "-s 2"
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test41.csv" -c $pvalue -r $refGen -p $pop -k "HI" -e "South Asian" -s "1"

    # TODO test 42 (test 39 without internet)

    # test 43 (-i, -e)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test43.csv" -c $pvalue -r $refGen -p $pop -i "GCST000001" -e "East Asian"

    # test 44 (-i x2, -e x2)
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test44.csv" -c $pvalue -r $refGen -p $pop -i "GCST000001" -i "GCST000010" -e "east asian" -e "european"

    # test 45 (-i, -e, -s) #TODO Maddy, did you use "-s 1" or "-s 2"
    ./runPrsCLI.sh -f $inputFilePath -o "$outputFolder/test45.csv" -c $pvalue -r $refGen -p $pop -i "GCST000001" -e "South Asian" -s "1"
